    ...
    new Promise(async (resolve, reject) => {
      ...
      if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
        ...
        try {
          ...
          var youtubeVideoID = res.url.replace("https://www.youtube.com/watch?v=","")
          fetchVideoInfo(youtubeVideoID).then(function (videoInfo) {
          ...
          d = Number(videoInfo.duration);

          var h = Math.floor(d / 3600);
          var m = Math.floor(d % 3600 / 60);
          var s = Math.floor(d % 3600 % 60);

          hhmmss = ('0' + h).slice(-2) + ":" + ('0' + m).slice(-2) + ":" + ('0' + s).slice(-2);

          embed.addField("Duration", hhmmss);
                  ...
                  msg.channel.awaitMessages(filter, {
                    ...
                    .then(collected => {
                      ...
                      if (song_number >= 0) {
                        ...
                        var fetchVideoInfo = require('youtube-info');
                        var youtubeVideoID = videos[song_number].url.replace("https://www.youtube.com/watch?v=","")
                        fetchVideoInfo(youtubeVideoID).then(function (videoInfo) {
                        embed.setAuthor('Adding To Queue', client.user.avatarURL);
